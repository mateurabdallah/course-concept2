...
def parse_engagement_file(file_storage):
    ...
    try:
        # المحاولة الأولى باستخدام openpyxl/xlrd
        df = pd.read_excel(BytesIO(file_bytes), header=None)
    except Exception as e:
        raise Exception(f"تعذر قراءة ملف Excel ({filename}). التأكد من صيغة الملف xlsx/xls: {str(e)}")

    if df.empty:
        raise Exception("الملف الفارغ")
    ...
    for r in range(min(15, len(df))):
        ...
        if any(k in row_str.lower() for k in ["club", "نادي", "association"]):
    ...
    for idx in range(header_row_idx + 1, len(df)):
        ...
        if not nom or nom.lower() in ["nan", "none", "0"] or nom.isdigit():
            continue
